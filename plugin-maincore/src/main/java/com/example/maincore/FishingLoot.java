 ...
 * </ul>
 * ServerBridge carries a copy for farm-server (no lava/void there).
 */
public class FishingLoot implements Listener {
    ...
    public FishingLoot(MainCorePlugin plugin) {
        this.plugin = plugin;
    ...
    // ---------- track descriptions (shared with Job / BaitRodItem lore) ----------

    public static String abilityEffect(int level) {
        int[] r = RATIOS[Math.max(0, Math.min(RATIOS.length - 1, level))];
        return "물고기 " + r[0] + "% / 쓰레기 " + r[1] + "%" + (r[2] > 0 ? " / 보물 " + r[2] + "%" : "");
    }

    public static String baitEffect(int level) {
        return switch (Math.max(0, Math.min(5, level))) {
            case 0 -> "기본 물고기";
            case 1 -> "온도에 따른 물고기";
            case 2 -> "바다 물고기 추가";
            case 3 -> "사막/습지/창백한 정원 물고기 추가";
            case 4 -> "네더 용암 낚시 가능";
            default -> "엔드 허공 낚시 가능";
        };
    }
    ...
    public void onFish(PlayerFishEvent event) {
        ...
        boolean fisher = profile != null && profile.job() == Job.FISHER;
        int ability = fisher ? profile.level(0) : 0;
        ItemStack rod = rodInHand(player);
        int bait = fisher && plugin.getBaitRodItem().isSpecialRod(rod) && plugin.getJobManager().canUse(player, rod)
                ? profile.level(1) : -1;
